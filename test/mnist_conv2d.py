...
scope_t, conv2d_method, conv2d_rate = "tt", "tt", 0.1
results_t, params_t, hypers_t = Model(inputs = x_image, scope = scope_t, data_format = data_format,
  conv2d_method = conv2d_method, conv2d_rate = conv2d_rate)
...
file_t = folder_name + scope_t + str(conv2d_rate) + model_name

# lost function and other measures
mean_squared_error = tf.losses.mean_squared_error(results["h2c"], results_t["h2c"]) + \
  tf.losses.mean_squared_error(results["h3c"], results_t["h3c"])
...
with tf.Session() as sess:
  ...
  load_params_conv2d_tensor(sess, hypers["conv2"], hypers_t["conv2"], method = conv2d_method, rate = conv2d_rate)
  load_params_conv2d_tensor(sess, hypers["conv3"], hypers_t["conv3"], method = conv2d_method, rate = conv2d_rate)
  load_params_dense_tensor(sess, hypers["fc1"], hypers_t["fc1"])
  ...
  for epoch in range(1, epoch_num_1 + 1):
    ...
    for i in range(1, batch_num_train + 1):
      ...
      if i % 50 == 0:
        ...
        print("samples %d, mean squared error: %.5f, training accuracy: %.2f" % (i * batch_size, train_mean_squared_error, train_accuracy))
