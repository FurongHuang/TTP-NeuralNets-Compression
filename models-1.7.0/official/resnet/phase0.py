...
def main(argv):
    ...
    parser.add_argument(
        '--filename', '-fn', default = 'normal_weights.ckpt.meta',
        help = "[default: %(default)s] The filename of the latest checkpoint of pretrained model.",
        metavar = '<FN>'
    )
    
    # Set defaults that are reasonable for this model.
    parser.set_defaults(data_dir='/tmp/cifar10_data',
                    pretrained_model_dir='/tmp/cifar10_model_tensor_based/normal/', # our resnet model, not the official one
                    resnet_size=32,
                    batch_size=128,
                    version=2,
                    output_path='/tmp/cifar10_model_tensor_based', 
                    filename='normal_weights.ckpt.meta',
                    method='svd',
                    scope='svd',
                    rate=0.5)
    ...
    saver = tf.train.import_meta_graph("%s/%s" %(pretrained_model_dir, filename))
